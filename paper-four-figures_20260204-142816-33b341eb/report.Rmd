...
vaccine_colours <- c("black", "deeppink4", "deeppink", "yellow4", "dodgerblue",
...
disease_colours <- c("black", "deeppink", "yellow4", "dodgerblue",
...
df2_mod_ave %>% filter(method == "standard", activity_type=="all") %>%
  fun_simple_ridges_alloutc(cols=disease_colours)
  
```


## Ladder subregion {.tabset}

### Standard (deaths)

```{r ladder_subregion}
ladsub <- df2_mod_ave %>% filter(method=="standard", activity_type=="all") %>%
  fun_ladder_subregion()
...
```{r ladder_subregion_dalys}
df2_mod_ave %>% filter(method=="standard", activity_type=="all") %>%
  fun_ladder_subregion(outc="dalys")

```


## Ladder subregion activity {.tabset}

### Standard (deaths)

```{r ladder_subregion_activity}
df2_mod_ave %>% filter(method=="standard", activity_type !="all") %>%
  filter(disease != "Rubella") %>%
  fun_ladder_subregion(act=TRUE)

```

### Standard (DALYs)

```{r ladder_subregion_activity_dalys}
df2_mod_ave %>% filter(method=="standard", activity_type !="all") %>%
  filter(disease != "Rubella") %>%
  fun_ladder_subregion(outc="dalys", act=TRUE)

```


## Ladder disease {.tabset}
...
```{r ladder_disease_activity}
df2_mod_ave %>% plot_ladder_disease_act()
```

### Standard (DALYs)

```{r ladder_disease_dalys}
df2_mod_ave %>% filter(method=="standard", activity_type=="all") %>%
  plot_ladder_disease(outc="dalys")
```

### Standard activity (DALYs)

```{r ladder_disease_activity_dalys}
df2_mod_ave %>% plot_ladder_disease_act(outc = "dalys")
```
...
df_men %>%
  ...
  group_by(vaccine,  subregion, activity_type,  run_id, method) %>%
...
## Modelling group variation  {.tabset}

### Deaths

```{r modelling_group_variation}
df3 <- oot %>% filter(method=="standard") %>% select(disease, modelling_group) %>% 
  distinct() %>%
  group_by(disease) %>% mutate(mod_num = row_number())

plot_modelling_group_variation(oot %>% filter(method=="standard") ,df3)
```

### DALYs 
```{r modelling_group_variation_dalys}
plot_modelling_group_variation(oot %>% filter(method=="standard"),df3, "dalys")
```
